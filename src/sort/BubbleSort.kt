...
class BubbleSort {
    ...
    fun bubbleSort(args: Array<String>) {
        var numSwaps = 0
        var isSorted = false
        val str = readLine()!!
        val intList: ArrayList<Int> = ArrayList(
            str.split(" ").map { it.toInt() }) //1) read values and convert them to arraylist
        ...
        while (!isSorted) {
            ...
            for (i in 0 until lastUnsorted) {
                if (intList[i] > intList[i + 1]) {
                    ...
                    numSwaps++
                    isSorted =
                        false // 3) making this as false whenever the swap needs to be performed.
                }
            }
